...
class SniperOMancer:
    ...
    def __init__(self):
        ...
        self.spend = self.Web3.to_checksum_address("0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c")  # wbnb contract address
    ...
    def get_balance(self):
        mybalance = self.Web3.eth.get_balance(wallet_address)
        return self.Web3.from_wei(mybalance, 'ether')
        ...
        while True:
            try:
                if self.reset_done:
                    ...
                    try:
                        event_filter = contract.events.PairCreated.create_filter(fromBlock='latest').get_new_entries()
                    except requests.exceptions.HTTPError:
                        event_filter = None

                    if event_filter is not None:
                        for PairCreated in event_filter:
                            jsonEventContents = json.loads(Web3.to_json(PairCreated))
    ...
    def place_order(self, contract, action):
        try:
            txn = None
            contract = self.Web3.to_checksum_address(contract)
            ...
            if action == 'buy':
                txn = self.pcs_contract.functions.swapExactETHForTokens(
                    ...
                    'gasPrice': self.Web3.to_wei(txn_speed, 'gwei'),
                    'nonce': nonce,
                })
            elif action == 'sell':
                txn = self.pcs_contract.functions.swapExactETHForTokens(
            ...
            receipt = self.Web3.eth.wait_for_transaction_receipt(signed_txn.hash)
            if receipt['status'] == 0:
